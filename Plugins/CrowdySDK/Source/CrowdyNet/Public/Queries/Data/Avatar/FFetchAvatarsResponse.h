...
struct FFetchAvatarsResponse : ICrowdyQueryResponse
{
	...
	virtual void ParseResponse(const TSharedPtr<FJsonObject>& JsonObject) override
	{
		...
		for (const TSharedPtr<FJsonValue>& AvatarValue : *MyAvatarsArray)
		{
			...
			{
				UE_LOG(LogCrowdyNet, Warning, TEXT("FetchAvatars: Missing avatarId field"));
				continue;
			}
			...
			{
				UE_LOG(LogCrowdyNet, Warning, TEXT("FetchAvatars: Missing name field for avatar %lld"), Raw.ID);
				Raw.Name = TEXT("");
			}
			...
			UE_CLOG(CrowdyNetTrace::Query(), LogCrowdyNet, Log, TEXT("FetchAvatars: Parsed raw avatar - ID: %lld, Name: %s"),
