...
struct FJoinTeamResponse : ICrowdyQueryResponse
{
	FCrowdyGroupMember Member;

	virtual EQueryResponseType GetResponseType() const override { return EQueryResponseType::JoinTeam; }
	virtual FName GetOperationName() const override { return "Join Team Response"; }
	...
	virtual void ParseResponse(const TSharedPtr<FJsonObject>& JsonObject) override
	{
		...
		if (!(*DataObj)->TryGetObjectField(TEXT("joinTeam"), Obj)) { bIsValid = false; return; }
