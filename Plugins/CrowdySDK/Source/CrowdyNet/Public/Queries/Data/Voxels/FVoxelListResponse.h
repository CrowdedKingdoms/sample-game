...
struct FVoxelListResponse : ICrowdyQueryResponse
{
	...
	virtual void ParseResponse(const TSharedPtr<FJsonObject>& JsonObject) override
	{
		...
		for (auto& VoxelData : *VoxelListArray)
		{
			...
			{
				UE_LOG(LogCrowdyNet, Warning, TEXT("Coordinates not found in voxel list element object"));
				continue;
			}
			...
			{
				UE_LOG(LogCrowdyNet, Warning, TEXT("Failed to extract core voxel list item(s)"));
			}
