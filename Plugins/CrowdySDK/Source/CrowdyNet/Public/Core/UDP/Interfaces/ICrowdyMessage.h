...
protected:
	...
	FORCEINLINE [[nodiscard]] bool DeserializeMetadata(const TArray<uint8>& Data, int32& Offset)
	{
		...
		{
			UE_LOG(LogCrowdyNet, Error, TEXT("DeserializeMetadata: Failed to deserialize MapID"));	
			return false;
		}
		...
		{
			UE_LOG(LogCrowdyNet, Error, TEXT("DeserializeMetadata: Failed to deserialize ChunkX"));	
			return false;
		}
		...
		{
			UE_LOG(LogCrowdyNet, Error, TEXT("DeserializeMetadata: Failed to deserialize ChunkY"))
			return false;
		}
		...
		{
			UE_LOG(LogCrowdyNet, Error, TEXT("DeserializeMetadata: Failed to read ChunkZ at offset %d"), LocalOffset);
			return false;
		}
		LocalOffset += sizeof(int64);
...
private:
	...
	FORCEINLINE [[nodiscard]] bool ExtractTailData(const TArray<uint8>& Data)
	{
		...
		{
			UE_LOG(LogCrowdyNet, Error, TEXT("ExtractTailData: Failed to read Timestamp at offset %d"), TempOffset);
			return false;
		}
