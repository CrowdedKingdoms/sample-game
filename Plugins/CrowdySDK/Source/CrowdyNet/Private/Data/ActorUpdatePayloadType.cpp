...
void UActorUpdatePayloadType::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	...
	for (int32 i = 0; i < Entries.Num(); i++)
	{
		...
		if (SeenIDs.Contains(ID))
		{
			...
			UE_LOG(LogCrowdyNet, Warning, TEXT("[UActorUpdatePayloadType]: Duplicate TypeID %d at index %d - reassigned to %d"),
				ID, i, Next);
		...
		if (SeenNames.Contains(Entries[i].ActorUpdateName))
		{
			...
			UE_LOG(LogCrowdyNet, Warning, TEXT("[UActorUpdatePayloadType]: Duplicate ActorUpdateName '%s' at index %d - reassigned to '%s'"),
