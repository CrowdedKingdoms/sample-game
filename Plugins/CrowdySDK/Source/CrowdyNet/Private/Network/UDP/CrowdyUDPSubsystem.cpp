...
void UCrowdyUDPSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	{
		UE_LOG(LogCrowdyNet, Error, TEXT("Failed to get CrowdyGameSession instance"));
	}
...
void UCrowdyUDPSubsystem::HandleUDPMessage(const uint8* Data, const int32 Size)
{
	...
	{
		UE_LOG(LogCrowdyNet, Warning, TEXT("HMAC verification failed for received message"));
		return;
	}
...
void UCrowdyUDPSubsystem::StartTimeoutMonitoring(const float ThresholdSeconds)
{
	...
	UE_CLOG(CrowdyNetTrace::Net(), LogCrowdyNet, Log, TEXT("UDP timeout monitoring started with threshold of %.2f seconds"), ThresholdSeconds);
	});
}
...
void UCrowdyUDPSubsystem::StopTimeoutMonitoring()
{
	...
	if (const UWorld* World = GetWorld())
	{
		...
		UE_CLOG(CrowdyNetTrace::Net(), LogCrowdyNet, Log, TEXT("UDP timeout monitoring stopped"));
	}
}
...
bool UCrowdyUDPSubsystem::InitializeUDP(const FUDPAddressNotify& UDPAddressNotify)
{
	...
	case ECrowdyUDPProtocol::IPv4:
		UE_CLOG(CrowdyNetTrace::Net(), LogCrowdyNet, Log, TEXT("UDP: protocol forced to IPv4"));
	...
	case ECrowdyUDPProtocol::IPv6:
		UE_CLOG(CrowdyNetTrace::Net(), LogCrowdyNet, Log, TEXT("UDP: protocol forced to IPv6"));
		bInitialised = InitializeUDPSocket(UDPAddressNotify.IPv6Address, UDPAddressNotify.Port);
		break;

	case ECrowdyUDPProtocol::Auto:
	default:
		UE_CLOG(CrowdyNetTrace::Net(), LogCrowdyNet, Log, TEXT("UDP: protocol auto — trying IPv6 first"));
		...
		{
			UE_CLOG(CrowdyNetTrace::Net(), LogCrowdyNet, Log, TEXT("UDP: IPv6 failed, falling back to IPv4"));
			if (InitializeV4UDPSocket(UDPAddressNotify.IPv4Address, UDPAddressNotify.Port))
			{
				bUseIPv4 = true;
				bInitialised = true;
...
void UCrowdyUDPSubsystem::CheckForTimeout()
{
	...
	{
		UE_LOG(LogCrowdyNet, Warning, TEXT("UDP timeout detected - no messages received for %lld seconds"),
...
bool UCrowdyUDPSubsystem::InitializeUDPSocket(const FString& IPAddress, const int32 Port)
{
	...
	if (!bIsValidAddress)
	{
		UE_LOG(LogCrowdyNet, Error, TEXT("Invalid IP address: %s"), *IPAddress);
		CleanupSockets();
		return false;
	}
	...
	if (!UDPSocketV6)
	{
		UE_LOG(LogCrowdyNet, Error, TEXT("Failed to create UDP socket"));
		return false;
	}
	...
	if (bIsIPv6)
	{
		...
		if (!bIsValid)
		{
			UE_LOG(LogCrowdyNet, Error, TEXT("Failed to set IPv6 any address"));
			CleanupSockets();
			return false;
		}
	}
	...
	{
		UE_LOG(LogCrowdyNet, Error, TEXT("Failed to bind UDP socket to local address"));
	...
	{
		UE_LOG(LogCrowdyNet, Error, TEXT("Failed to connect UDP socket to %s:%d"), *IPAddress, Port);
		// Cleanup socket inline
		CleanupSockets();
		return false;
	}
	...
	UE_CLOG(CrowdyNetTrace::Net(), LogCrowdyNet, Log, TEXT("UDP (%s) buffers set: snd=%d, rcv=%d"),
		   bIsIPv6 ? TEXT("IPv6") : TEXT("IPv4"), NewSendBufferSize, NewRecvBufferSize);

	UE_CLOG(CrowdyNetTrace::Net(), LogCrowdyNet, Log, TEXT("Successfully connected UDP socket (%s) to %s:%d"),
...
bool UCrowdyUDPSubsystem::InitializeV4UDPSocket(const FString& IPAddress, const int32 Port)
{
	...
	if (!SocketSubsystem)
	{
		UE_LOG(LogCrowdyNet, Error, TEXT("Failed to get socket subsystem"));
		return false;
	}
	...
	if (!UDPSocketV4)
	{
		UE_LOG(LogCrowdyNet, Error, TEXT("Failed to create UDP socket (IPv4)"));
		return false;
	}
	...
	if (!bIsValidAddress)
	{
		UE_LOG(LogCrowdyNet, Error, TEXT("Invalid IPv4 address: %s"), *IPAddress);
		// Cleanup socket inline
		CleanupSockets();
		return false;
	}
	...
	{
		UE_LOG(LogCrowdyNet, Error, TEXT("Failed to bind UDP socket (IPv4) to local address"));
		// Cleanup socket inline
		CleanupSockets();
		return false;
	}
	...
	{
		UE_LOG(LogCrowdyNet, Error, TEXT("Failed to connect UDP socket (IPv4) to %s:%d"), *IPAddress, Port);
		// Cleanup socket inline
		CleanupSockets();
		return false;
	}
	...
	UE_CLOG(CrowdyNetTrace::Net(), LogCrowdyNet, Log, TEXT("UDP (IPv4) buffers set: snd=%d, rcv=%d"), NewSendBufferSize, NewRecvBufferSize);

	UE_CLOG(CrowdyNetTrace::Net(), LogCrowdyNet, Log, TEXT("Successfully connected UDP socket (%s) to %s:%d"), TEXT("IPv4"), *IPAddress, Port);
...
bool UCrowdyUDPSubsystem::SendUDPv6(const TArray<uint8>& Message)
{
	...
	if (bUDPPacketSent && BytesSentNow > 0)
	{
		...
		{
			//UE_LOG(LogCrowdyNet, Log, TEXT("Client Event Notify Sent"));
	...
	if (!SocketSubsystem)
	{
		UE_LOG(LogCrowdyNet, Error, TEXT("UDP (IPv6) packet send failed - unable to get error details"));
		return false;
	}

	const ESocketErrors SocketError = SocketSubsystem->GetLastErrorCode();
	UE_LOG(LogCrowdyNet, Error, TEXT("UDP (IPv6) packet send failed with socket error: %d"), (int32)SocketError);
	
	switch (SocketError)
	{
	case SE_NO_ERROR:
		UE_LOG(LogCrowdyNet, Warning, TEXT("UDP (IPv6) packet send completed but no bytes sent"));
		break;
	case SE_EWOULDBLOCK:
		UE_LOG(LogCrowdyNet, Warning, TEXT("UDP (IPv6) send would block - socket buffer full"));
		break;
	case SE_ECONNRESET:
		UE_LOG(LogCrowdyNet, Error, TEXT("UDP (IPv6) connection reset by peer"));
		break;
	case SE_ENETDOWN:
		UE_LOG(LogCrowdyNet, Error, TEXT("UDP (IPv6) network is down"));
		break;
	case SE_EHOSTUNREACH:
		UE_LOG(LogCrowdyNet, Error, TEXT("UDP (IPv6) host unreachable"));
		break;
	case SE_EMSGSIZE:
		UE_LOG(LogCrowdyNet, Error, TEXT("UDP (IPv6) message too large"));
		break;
	default:
		UE_LOG(LogCrowdyNet, Error, TEXT("UDP (IPv6) packet send failed with socket error: %d"),
			   (int32)SocketError);
	}
...
bool UCrowdyUDPSubsystem::SendUDPv4(const TArray<uint8>& Message)
{
	...
	if (!SocketSubsystem)
	{
		UE_LOG(LogCrowdyNet, Error, TEXT("UDP (IPv4) packet send failed - unable to get error details"));
		return false;
	}

	const ESocketErrors SocketError = SocketSubsystem->GetLastErrorCode();
	UE_LOG(LogCrowdyNet, Error, TEXT("UDP (IPv4) packet send failed with socket error: %d"), (int32)SocketError);

	switch (SocketError)
	{
	case SE_NO_ERROR:
		UE_LOG(LogCrowdyNet, Warning, TEXT("UDP (IPv4) packet send completed but no bytes sent"));
		break;
	case SE_EWOULDBLOCK:
		UE_LOG(LogCrowdyNet, Warning, TEXT("UDP (IPv4) send would block - socket buffer full"));
		break;
	case SE_ECONNRESET:
		UE_LOG(LogCrowdyNet, Error, TEXT("UDP (IPv4) connection reset by peer"));
		break;
	case SE_ENETDOWN:
		UE_LOG(LogCrowdyNet, Error, TEXT("UDP (IPv4) network is down"));
		break;
	case SE_EHOSTUNREACH:
		UE_LOG(LogCrowdyNet, Error, TEXT("UDP (IPv4) host unreachable"));
		break;
	case SE_EMSGSIZE:
		UE_LOG(LogCrowdyNet, Error, TEXT("UDP (IPv4) message too large"));
		break;
	default:
		UE_LOG(LogCrowdyNet, Error, TEXT("UDP (IPv4) packet send failed with socket error: %d"),
			   (int32)SocketError);
	}
...
void UCrowdyUDPSubsystem::StopUDPListener()
{
	...
	UE_CLOG(CrowdyNetTrace::Net(), LogCrowdyNet, Log, TEXT("UDP IPv6 Listener Loop Stopped"));
}
