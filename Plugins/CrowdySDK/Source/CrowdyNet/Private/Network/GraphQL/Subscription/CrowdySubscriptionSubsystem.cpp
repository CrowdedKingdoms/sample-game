...
void UCrowdySubscriptionSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	UE_CLOG(CrowdyNetTrace::Query(), LogCrowdyNet, Log, TEXT("[CrowdySubscriptions] Subsystem initialised."));
}
...
void UCrowdySubscriptionSubsystem::Connect(const FString& AuthToken)
{
	...
	{
		UE_CLOG(CrowdyNetTrace::Query(), LogCrowdyNet, Log, TEXT("[CrowdySubscriptions] Already connected/connecting — ignoring Connect()."));
		return;
	}
...
void UCrowdySubscriptionSubsystem::Disconnect()
{
	...
	SetState(ESubscriptionConnectionState::Idle);
	UE_CLOG(CrowdyNetTrace::Query(), LogCrowdyNet, Log, TEXT("[CrowdySubscriptions] Disconnected."));
}
	...
	ICrowdySubscriptionHandler* Handler)
{
	if (!Handler)
	{
		UE_LOG(LogCrowdyNet, Warning, TEXT("[CrowdySubscriptions] Subscribe() called with a null handler — ignored."));
		return FCrowdySubscriptionHandle::Invalid();
	}

	if (Query.IsEmpty())
	{
		UE_LOG(LogCrowdyNet, Warning, TEXT("[CrowdySubscriptions] Subscribe() called with an empty query — ignored."));
		return FCrowdySubscriptionHandle::Invalid();
	}
...
void UCrowdySubscriptionSubsystem::Unsubscribe(const FCrowdySubscriptionHandle& Handle)
{
	...
	{
		UE_LOG(LogCrowdyNet, Warning, TEXT("[CrowdySubscriptions] Unsubscribe: handle '%s' not found."), *Handle.ID);
		return;
	}
	...
	UE_CLOG(CrowdyNetTrace::Query(), LogCrowdyNet, Log, TEXT("[CrowdySubscriptions] Unsubscribed '%s'."), *Handle.ID);
}
...
void UCrowdySubscriptionSubsystem::UnsubscribeAll()
{
	...
	ActiveSubscriptions.Empty();
	UE_CLOG(CrowdyNetTrace::Query(), LogCrowdyNet, Log, TEXT("[CrowdySubscriptions] All subscriptions cancelled."));
}
...
void UCrowdySubscriptionSubsystem::CreateAndConnectSocket()
{
	...
	//UE_CLOG(CrowdyNetTrace::Query(), LogCrowdyNet, Log, TEXT("[CrowdySubscriptions] Connecting to %s"), *Settings->GameApiWsUrl);

	// graphql-transport-ws is the modern GraphQL-over-WS sub-protocol.
	Socket = FWebSocketsModule::Get().CreateWebSocket(
		URL, TEXT("graphql-transport-ws"));
...
{
	UE_CLOG(CrowdyNetTrace::Query(), LogCrowdyNet, Log, TEXT("[CrowdySubscriptions] WebSocket connected — sending connection_init."));
...
{
	UE_LOG(LogCrowdyNet, Error, TEXT("[CrowdySubscriptions] WebSocket error: %s"), *Error);
...
void UCrowdySubscriptionSubsystem::HandleMessage(const FString& MessageText)
{
	...
	{
		UE_LOG(LogCrowdyNet, Warning, TEXT("[CrowdySubscriptions] Server message has no 'type' field."));
		return;
	}
	...
	else if (Type == TEXT("connection_error"))
	{
		...
		UE_LOG(LogCrowdyNet, Error, TEXT("[CrowdySubscriptions] connection_error from server: %s"), *Msg);
	}
	else
	{
		UE_CLOG(CrowdyNetTrace::Query(), LogCrowdyNet, Verbose, TEXT("[CrowdySubscriptions] Unhandled message type '%s'."), *Type);
	}
}
	...
	const FString& ID, const FSubscriptionEntry& Entry) const
{
	...
	SendRawJson(Msg);
	UE_CLOG(CrowdyNetTrace::Query(), LogCrowdyNet, Log, TEXT("[CrowdySubscriptions] Sent subscribe '%s'."), *ID);
}
...
{
	UE_CLOG(CrowdyNetTrace::Query(), LogCrowdyNet, Log, TEXT("[CrowdySubscriptions] Attempting reconnect..."));
	CreateAndConnectSocket();
}
