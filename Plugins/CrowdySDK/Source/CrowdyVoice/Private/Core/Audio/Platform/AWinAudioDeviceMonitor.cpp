...
void AWinAudioDeviceMonitor::InitializeAudioDeviceMonitoring()
{
    ...
    {
        UE_LOG(LogCrowdyVoice, Warning, TEXT("Audio device monitoring is already active"));
        return;
    }
    ...
    if (SUCCEEDED(hr))
    {
        ...
        if (SUCCEEDED(hr))
        {
            ...
            UE_CLOG(CrowdyVoiceTrace::Voice(), LogCrowdyVoice, Log, TEXT("Audio device monitoring initialized successfully"));
        }
        else
        {
            UE_LOG(LogCrowdyVoice, Error, TEXT("Failed to register for audio device notifications. Error code: %d"), hr);
        }
    }
    else
    {
        UE_LOG(LogCrowdyVoice, Error, TEXT("Failed to create audio device enumerator. Error code: %d"), hr);
    }
#else
    UE_LOG(LogCrowdyVoice, Warning, TEXT("Audio device monitoring is only supported on Windows"));
#endif
}
...
void AWinAudioDeviceMonitor::ShutdownAudioDeviceMonitoring()
{
    ...
    UE_CLOG(CrowdyVoiceTrace::Voice(), LogCrowdyVoice, Log, TEXT("Audio device monitoring shutdown"));
#endif
}
...
TArray<FString> AWinAudioDeviceMonitor::GetAvailableAudioDevices(bool bOutputDevices)
{
    ...
    if (!DeviceEnumerator)
    {
        UE_LOG(LogCrowdyVoice, Warning, TEXT("Device enumerator not initialized"));
        return DeviceList;
    }
